...
async function main():Promise<void>{
  ...
  await Promise.all([
    ...
    ensureDir(batchDirectory),

    ensureDir(finalVideosDirectory)
  ...
  if(config.topics.length > 0){
    ...
    console.log(
      `Generating ${config.batchCount} unique topics...`
    );
    ...
    try {
      ...
      const deliveryPath =
        path.join(
          ...
          finalVideosDirectory,

          `${sequence}-${cleanName}.mp4`
      ...
      console.log(
        `Completed: ${result.blueprint.title}`
      );


      console.log(
        `Final video prepared: ${deliveryPath}`
      );
  ...
  console.log(
    `Download-ready videos: ${finalVideosDirectory}`
  );
  ...
  if(completed.length===0){
    ...
    throw new Error(
      'All videos failed. Check batch-manifest.json'
    );
